...
if ($stmt1->execute()) {
    ...
    $stmt3 = $conn->prepare("UPDATE `savings_account` SET `account_bal`=$new_bal,`last_transaction_amount`=$amount,`last_transaction_type`='Deposit',`last_transaction_date`='$today' WHERE account_number='$acc_no'");
    $stmt3->execute();
    header("Location: ../../Admin/Pages/SavingsAccountSummaryPage.php?memberId=$memberId&msg=<p class='bg-success-subtle text-success p-2 rounded text-center'>Amount of Rs. <span class='fw-bold'>" . $amount . "</span> deposited into Account <span class='fw-bold'>" . $acc_no . "</span> Successfully.</p>");
}
